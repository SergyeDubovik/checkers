...
public class Move {
     ...
     * (Get in between jumps using recursion)
     *
    ...
    public Piece[] getJumpedPieces(Board board) {
        ...
        if (isJump) {
            // create an expandable list of all pieces
            ...
            // ...but also go back to get the in between ones (if we're not the first move)
            ...
            // shorten and return
            pieces.trimToSize();
            return pieces.toArray(new Piece[1]); // convert to a Piece array
        } else
            return null;
    }
}
